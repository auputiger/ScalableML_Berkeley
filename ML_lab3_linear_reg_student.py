...
numPoints = rawData.count() #<FILL IN>
print numPoints
samplePoints = rawData.take(5) #<FILL IN>
print samplePoints
...
#parsedSamplePoints = rawData.map(parsePoint).take(5) 
parsedSamplePoints = map(parsePoint, samplePoints) #<FILL IN>
#print parsedSamplePoints

firstPointFeatures = parsedSamplePoints[0].features #<FILL IN>
firstPointLabel = parsedSamplePoints[0].label #<FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
parsedDataInit = rawData.map(parsePoint) #(<FILL IN>)
onlyLabels = parsedDataInit.map(lambda lp: lp.label) #(<FILL IN>)
minYear = onlyLabels.min() #<FILL IN>
maxYear = onlyLabels.max() #<FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
parsedData = parsedDataInit.map(lambda lp: LabeledPoint(lp.label - minYear, lp.features)) #<FILL IN>
...
weights = [.8, .1, .1]
seed = 42
parsedTrainData, parsedValData, parsedTestData = parsedData.randomSplit(weights, seed) #<FILL IN>
parsedTrainData.cache() #<FILL IN>
parsedValData.cache() #<FILL IN>
parsedTestData.cache() #<FILL IN>
nTrain = parsedTrainData.count() #<FILL IN>
nVal = parsedValData.count() #<FILL IN>
nTest = parsedTestData.count() #<FILL IN>
...
def calcRMSE(labelsAndPreds):
    ...
    """
    #return 
    SqErrList = labelsAndPreds.map(lambda (label, prediction): squaredError(label, prediction)).collect()
    MSE = reduce(lambda a, b: a+b, SqErrList) / len(SqErrList) #sum(SqErrList) 
    return np.sqrt(MSE)
    #<FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda lp: (lp.label, averageTrainYear)) #<FILL IN>
rmseTrainBase = calcRMSE(labelsAndPredsTrain) #<FILL IN>

labelsAndPredsVal = parsedValData.map(lambda lp: (lp.label, averageTrainYear)) #<FILL IN>
rmseValBase = calcRMSE(labelsAndPredsVal) #<FILL IN>

labelsAndPredsTest = parsedTestData.map(lambda lp: (lp.label, averageTrainYear)) #<FILL IN>
rmseTestBase = calcRMSE(labelsAndPredsTest) #<FILL IN>
...
def getLabeledPrediction(weights, observation):
    ...
    x = DenseVector(observation.features)
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        #.map(lambda lp: (lp.label, DenseVector(lp.features).dot(w))) #<FILL IN>
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)

        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a).
        # Note that `gradient` sould be a `DenseVector` of length `d`.
        gradient = trainData.map(lambda lp: gradientSummand(w, lp)).reduce(lambda a, b: a+b) #<FILL IN>

        # Update the weights
        alpha_i = alpha / (n * np.sqrt(i+1))
        w -= alpha_i * gradient #<FILL IN>
    return w, errorTrain
...
# TODO: Replace <FILL IN> with appropriate code
numIters = 50
weightsLR0, errorTrainLR0 = linregGradientDescent(parsedTrainData, numIters) #(<FILL IN>)

labelsAndPreds = parsedValData.map(lambda lp: getLabeledPrediction(weightsLR0, lp)) #<FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
#train(data, iterations=100, step=1.0, miniBatchFraction=1.0, 
#    initialWeights=None, regParam=0.0, regType=None, intercept=False, validateData=True)
firstModel = LinearRegressionWithSGD.train(parsedTrainData, iterations = numIters, step = alpha, 
                                           miniBatchFraction = miniBatchFrac, initialWeights =None, 
                                           regParam = reg, regType = regType, intercept = useIntercept) #<FILL IN>

# weightsLR1 stores the model weights; interceptLR1 stores the model intercept
weightsLR1 = firstModel.weights #<FILL IN>
interceptLR1 = firstModel.intercept #<FILL IN>
...
#print samplePoint
#samplePrediction = firstModel.predict(np.array(samplePoint.features)) #<FILL IN>
samplePrediction = firstModel.predict(samplePoint.features) #<FILL IN>
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPreds = parsedValData.map(lambda lp: (lp.label, firstModel.predict(lp.features))) #<FILL IN>
rmseValLR1 = calcRMSE(labelsAndPreds) #<FILL IN>
...
alpha = 1.0
miniBatchFrac = 1.0
for reg in [1e-10, 1e-5, 1]: #<FILL IN>:
...
for alpha in [1e-5, 10]: #<FILL IN>:
    for numIters in [500, 5]: #<FILL IN>:
        model = LinearRegressionWithSGD.train(parsedTrainData, numIters, alpha,
                                              miniBatchFrac, regParam=reg,
                                              regType='l2', intercept=True)
        labelsAndPreds = parsedValData.map(lambda lp: (lp.label, model.predict(lp.features)))
...
def twoWayInteractions(lp):
    ...
    #<FILL IN>
    miao = lp.features
    miaomiao = list(itertools.product(miao, miao))
    mi = [x*y for (x, y) in miaomiao]
    newFeatures = np.hstack((miao, mi))
    return LabeledPoint(lp.label, newFeatures)

print twoWayInteractions(LabeledPoint(0.0, [2, 3]))

# Transform the existing train, validation, and test sets to include two-way interactions.
trainDataInteract = parsedTrainData.map(lambda lp: twoWayInteractions(lp)) #<FILL IN>
valDataInteract = parsedValData.map(lambda lp: twoWayInteractions(lp)) #<FILL IN>
testDataInteract = parsedTestData.map(lambda lp: twoWayInteractions(lp)) #<FILL IN>
...
modelInteract = LinearRegressionWithSGD.train(trainDataInteract, numIters, alpha, #<FILL IN>, numIters, alpha,
                                              miniBatchFrac, regParam=reg,
                                              regType='l2', intercept=True)
labelsAndPredsInteract = valDataInteract.map(lambda lp: (lp.label, modelInteract.predict(lp.features)))
                    #<FILL IN>.map(lambda lp: (lp.label, <FILL IN>.predict(lp.features)))
...
labelsAndPredsTest = testDataInteract.map(lambda lp: (lp.label, modelInteract.predict(lp.features))) #<FILL IN>
rmseTestInteract = calcRMSE(labelsAndPredsTest) #<FILL IN>
